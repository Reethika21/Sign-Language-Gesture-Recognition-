...
# Updated file paths
filename = r'C:\Users\HP\Desktop\market segmentation\final_model.sav'
loaded_model = pickle.load(open(filename, 'rb'))
df = pd.read_csv(r"C:\Users\HP\Desktop\market segmentation\Clustered_Customer_Data.csv")
...
if submitted:
    ...
    # Adjusted plotting code for Streamlit
    st.subheader("Cluster Distribution")
    for c in cluster_df1.drop(['Cluster'], axis=1):
        plt.figure(figsize=(10, 4))
        sns.histplot(cluster_df1[c], bins=30, kde=True)
        plt.title(f'Distribution of {c}')
        plt.xlabel(c)
        plt.ylabel('Frequency')
        st.pyplot(plt)  # Streamlit-compatible plotting
